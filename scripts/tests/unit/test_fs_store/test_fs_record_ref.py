...
class TestConstruction:
    ...
    def test_with_record_path(self):
        ref = RecordRef(id="b", type="session", record_path="/tmp/b.json")
...
class TestToDict:
    ...
    def test_includes_record_path_when_set(self):
        d = RecordRef(id="a", type="task", record_path="/p").to_dict()
...
class TestFromDict:
    ...
    def test_with_record_path(self):
        ref = RecordRef.from_dict({"id": "x", "type": "t", "record_path": "/p"})
...
class TestRoundTrip:
    ...
    def test_round_trip_with_path(self):
        original = RecordRef(id="b", type="session", record_path="/tmp/b.json")
        restored = RecordRef.from_dict(original.to_dict())
        assert restored.id == original.id
        assert restored.type == original.type
