---
description: Create a persistent memory entry from a session insight. Use when a session reveals a user preference, project convention, or knowledge that should be remembered across sessions.
...
1. **Report "creating" status**: Call the MCP `flow_entity_crud` tool with:
   - `crud`: "create"
   - `entity_json`: `{"type": "memory", "status": "creating", "description": "<brief description of the memory>"}`

Launch ONLY ONE agent: `skillit:skillit-memory-creator`. Do NOT launch any other agent.

Use the Task tool with EXACTLY these parameters:
- `subagent_type: "skillit:skillit-memory-creator"`
...
## Verify memory was installed
After relaying signals, verify the memory file was written to the expected location (`~/.claude/projects/{project}/memory/` or `~/.claude/memory/`).
