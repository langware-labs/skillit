---
description: Create a coding rule from a session insight. Use when a session reveals a constraint or coding standard that should be enforced.
...
1. **Report "creating" status**: Call the MCP `flow_entity_crud` tool with:
   - `crud`: "create"
   - `entity_json`: `{"type": "rule", "status": "creating", "description": "<brief description of the rule>"}`

Launch ONLY ONE agent: `skillit:skillit-rule-creator`. Do NOT launch any other agent.

Use the Task tool with EXACTLY these parameters:
- `subagent_type: "skillit:skillit-rule-creator"`
...
After relaying signals, verify the rule file was written to the expected location (`.claude/rules/` for project scope or `~/.claude/rules/` for user scope).
